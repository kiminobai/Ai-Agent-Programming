...
const chatHandler: RequestHandler<Record<string, string>, ChatResponsePayload | { error: string }, ChatRequestPayload> = async (
  req: Request<Record<string, string>, ChatResponsePayload | { error: string }, ChatRequestPayload>,
  res: Response<ChatResponsePayload | { error: string }>
    ...
    try {
      const reply = await provider.sendChat(model.id, userMessage);
      res.json({
        reply,
        meta: {
...
app.listen(appConfig.port, appConfig.host, () => {
  console.log(
    `Chat Demo is running at http://${appConfig.host}:${appConfig.port}`
  );
});
